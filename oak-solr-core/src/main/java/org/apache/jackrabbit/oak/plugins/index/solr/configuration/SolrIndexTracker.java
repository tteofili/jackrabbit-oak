...
public class SolrIndexTracker {

    /** Logger instance. */
    private static final Logger log = LoggerFactory.getLogger(SolrIndexTracker.class);
    private static final PerfLogger PERF_LOGGER =
            new PerfLogger(LoggerFactory.getLogger(SolrIndexTracker.class.getName() + ".perf"));

    private NodeState root = EMPTY_NODE;

    private volatile Map<String, SolrIndexNode> indices = emptyMap();

    SolrIndexTracker() {}

    synchronized void close() {
        Map<String, SolrIndexNode> indices = this.indices;
        this.indices = emptyMap();

        for (Map.Entry<String, SolrIndexNode> entry : indices.entrySet()) {
            try {
    ...
    synchronized void update(final NodeState root) {
        Map<String, SolrIndexNode> original = indices;
        final Map<String, SolrIndexNode> updates = newHashMap();

        List<Editor> editors = newArrayListWithCapacity(original.size());
        for (Map.Entry<String, SolrIndexNode> entry : original.entrySet()) {
            ...
            editors.add(new SubtreeEditor(new DefaultEditor() {
                ...
                public void leave(NodeState before, NodeState after) {
                    try {
                        ...
                        SolrIndexNode index = SolrIndexNode.open(path, root, after);
                        PERF_LOGGER.end(start, -1, "[{}] Index found to be updated. Reopening the SolrIndexNode", path);
        ...
        if (!updates.isEmpty()) {
            indices = ImmutableMap.<String, SolrIndexNode>builder()
            ...
            for (String path : updates.keySet()) {
                SolrIndexNode index = original.get(path);
                try {
                    index.close();
                } catch (IOException e) {
                    log.error("Failed to close Solr index at " + path, e);
                }
            }
        }
    }

    public SolrIndexNode acquireIndexNode(String path) {
        SolrIndexNode index = indices.get(path);
    ...
    private synchronized SolrIndexNode findIndexNode(String path) {
        ...
        try {
            if (isSolrIndexNode(node)) {
                index = SolrIndexNode.open(path, root, node);
                if (index != null) {
                    checkState(index.acquire());
                    indices = ImmutableMap.<String, SolrIndexNode>builder()
                            .putAll(indices)
