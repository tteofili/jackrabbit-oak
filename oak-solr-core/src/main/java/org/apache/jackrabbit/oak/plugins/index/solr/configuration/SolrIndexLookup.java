...
public class SolrIndexLookup {
    ...
    private static void collectIndexNodePaths(NodeState nodeState, String parentPath, Collection<String> paths) {
        ...
        for (ChildNodeEntry entry : state.getChildNodeEntries()) {
            if (isSolrIndex(entry.getNodeState())) {
                paths.add(createIndexNodePath(parentPath, entry.getName()));
            }
        }
    }

    public static boolean isSolrIndex(NodeState nodeState) {
        return "solr".equals(nodeState.getString(TYPE_PROPERTY_NAME));
    }
