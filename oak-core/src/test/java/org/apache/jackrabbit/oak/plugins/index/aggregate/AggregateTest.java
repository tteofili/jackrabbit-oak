...
public class AggregateTest {
    ...
    private Aggregate.Include ni(String pattern) {
        return new Aggregate.NodeInclude(mapper, pattern);
    }

    private Aggregate.Include ni(String type, String pattern, boolean relativeNode) {
        return new Aggregate.NodeInclude(mapper, type, pattern, relativeNode);
    }
    ...
    private static class TestCollector implements Aggregate.ResultCollector {
        ...
        public List<Aggregate.NodeIncludeResult> getRelativeNodeResults(String path, String rootIncludePath) {
            List<Aggregate.NodeIncludeResult> result = Lists.newArrayList();

            for (Aggregate.NodeIncludeResult nr : nodeResults.get(path)) {
